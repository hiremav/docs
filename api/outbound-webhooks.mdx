...
<Note>
  On **Mav.next**, webhook subscriptions are configured per agent as [lead boxes](/mav-next/lead-boxes) rather than on a playbook, and the catalog of events is documented in the [notification events reference](/mav-next/notification-events). The delivery mechanics and payload shape described below are the same.
</Note>
